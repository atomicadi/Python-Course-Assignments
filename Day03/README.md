...
## 3. Command-line
For the calculation of command line, external ```sys.argv``` library is employed where with the program file the user inputs are also provided. The file is saved as "q_trans_cmdline.py".
Command line to run the file:
```sh
